...
public class Human
{
    ...
    public const int BoneTypeCount = 11;

    private readonly Bone[] _bones = new Bone[BoneTypeCount].Fill();
    ...
    public Human()
    {
        for (int i = 0; i < BoneTypeCount; ++i)
        {
            _bones[i].BodyId = BodyId.NullId;
            _bones[i].JointId = JointId.NullId;
            _bones[i].FrictionScale = 1.0f;
            _bones[i].ParentIndex = -1;
        }
    ...
    public void ApplyRandomAngularImpulse(float magnitude)
    {
        if (IsSpawned == false)
        {
            return;
        ...
        var impulse = B2Random.Shared.RandomFloat(-magnitude, magnitude);
        Body.ApplyAngularImpulse(_bones[BoneType.Torso].BodyId, impulse, true);
    }

    public void SetJointFrictionTorque(float torque)
    {
        if (IsSpawned == false)
        {
            return;
        ...
        if (torque == 0.0f)
        {
            for (int i = 1; i < BoneTypeCount; ++i)
            {
                RevoluteJointFunc.EnableMotor(_bones[i].JointId, false);
            }
        }
        else
        {
            for (int i = 1; i < BoneTypeCount; ++i)
            {
                RevoluteJointFunc.EnableMotor(_bones[i].JointId, true);
                float scale = Scale * _bones[i].FrictionScale;
                RevoluteJointFunc.SetMaxMotorTorque(_bones[i].JointId, scale * torque);
            }
        }
    }

    public void SetJointSpringHertz(float hertz)
    {
        if (IsSpawned == false)
        {
            return;
        ...
        if (hertz == 0.0f)
        {
            for (int i = 1; i < BoneTypeCount; ++i)
            {
                RevoluteJointFunc.EnableSpring(_bones[i].JointId, false);
            }
        }
        else
        {
            for (int i = 1; i < BoneTypeCount; ++i)
            {
                RevoluteJointFunc.EnableSpring(_bones[i].JointId, true);
                RevoluteJointFunc.SetSpringHertz(_bones[i].JointId, hertz);
            }
        }
    }

    public void SetJointDampingRatio(float dampingRatio)
    {
        if (IsSpawned == false)
        {
            return;
        ...
        for (int i = 1; i < BoneTypeCount; ++i)
        {
            RevoluteJointFunc.SetSpringDampingRatio(_bones[i].JointId, dampingRatio);
        }
    }
}